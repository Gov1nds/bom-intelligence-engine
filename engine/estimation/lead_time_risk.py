"""Lead-time estimation based on procurement class, quantity, and category."""
import logging

logger = logging.getLogger("lead_time_estimator")

# Base lead times in days by procurement class
BASE_LEAD_TIMES = {
    "catalog_purchase": (3, 14),
    "custom_fabrication": (15, 45),
    "raw_material_order": (7, 21),
    "subassembly": (20, 60),
    "unknown": (7, 30),
}
...
RISK_FACTORS = {
    ...
    "custom_part": {"weight": 0.20, "triggers": True},
    "no_mpn": {"weight": 0.10, "triggers": True},
...
def estimate_risk(category: str, procurement_class: str, material: str, quantity: float,
                  is_custom: bool, has_mpn: bool, estimated_cost_mid: float, estimated_lead_mid: int) -> dict:
    flags = []
    total_risk = 0.0
