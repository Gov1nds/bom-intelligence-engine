...
FastAPI app + CLI entry. Run: python main.py --sample
"""
...
def cli():
    ...
    if args.memory:
        print(json.dumps(engine.memory_state(), indent=2)); return
    if args.serve:
        _serve(); return

    if args.sample:
        tmp = tempfile.NamedTemporaryFile(suffix=".csv", delete=False, mode="w")
        _write_csv(tmp.name, SAMPLE); tmp.close(); fp = tmp.name
    elif args.file:
        fp = args.file
        if not Path(fp).exists(): print(f"Not found: {fp}"); sys.exit(1)
    else:
    ...
    try:
        ...
        if args.output:
            with open(args.output, "w") as f: json.dump(report, f, indent=2, default=str)
            print(f"Report saved: {args.output}")
        else:
            s = report["section_1_executive_summary"]
            bd = s["cost_breakdown"]; lt = s["lead_time"]; opt = s["optimization"]; dd = s["decision_distribution"]
            print(f"\n{'='*60}\nEXECUTIVE SUMMARY\n{'='*60}")
        ...
        if args.sample and os.path.exists(fp): os.unlink(fp)
...
def _serve():
    ...
    @app.get("/")
    def root(): return {"service":"BOM Intelligence Engine","version":"2.0.0","status":"ok",
                         "memory":{"exploration_rate":engine.dm.exploration_rate,"confidence":engine.dm.confidence,"iterations":engine.dm.iterations}}
    @app.get("/health")
    def health(): return {"status":"ok","ts":datetime.now().isoformat()}
    @app.post("/api/analyze-bom")
    async def analyze(file: UploadFile = File(...), user_location: str = Form(""), target_currency: str = Form("USD")):
        sp = Path("uploads") / f"{uuid.uuid4().hex[:8]}_{file.filename}"
        with open(sp,"wb") as f: shutil.copyfileobj(file.file, f)
        try: return JSONResponse(engine.run_pipeline(str(sp), user_location, target_currency))
        finally: sp.unlink(missing_ok=True)
    @app.get("/api/memory")
    def mem(): return engine.memory_state()
